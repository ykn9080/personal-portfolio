  ...
  arr={[
    {
      ...
      label: "Photos",
      content: (
        <SearchShow id="dvela1" script="ls -la" script1="ls -la"></SearchShow>
      ),
    },
    {
      id: "music",
...
/>
## Elastic Search Tagging Process

## index mappings update
...
### 2. redefine mappings of nginxlog data
...
<SearchScript id="dvela4" filename="mappings_update" type="json" />
...
<SearchScript id="dvela6" filename="index_template" script1="ls -la" />

### 5. apply index template to reindex

- 재정의된 nginxlog를 dest로 source를 reindex한다.
- 하나의 index만 테스트해본다.

<SearchScript id="dvela7" filename="reindex_template" script1="ls -la" />

### 6. painless script

- 날짜로 구분된 동일 mapping을 가진 복수의 index를 수정하기 위한 script작성
- nginxlog-날짜 형태의 index를 loop돌면서 nginxlog mappings로 reindex한다.
...
<SearchScript id="dvela9" filename="pipeline" script1="ls -la" />
